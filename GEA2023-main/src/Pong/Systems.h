...
class EnemySetupSystem : public SetupSystem {
  public:
    void run() {
      ...
      scene->enemy = new Entity(scene->r.create(), scene);
      scene->enemy->addComponent<TransformComponent>(150, 150);
      auto& enemy1 = scene->enemy->addComponent<SpriteComponent>(
        "Sprites/Cat/SpriteSheet.png",
        0, 6,
        spriteSize,
        4,
      ...
      enemy1.lastUpdate = SDL_GetTicks();
      scene->enemy->addComponent<ColliderComponent>(19, 22, 10, 10, SDL_Color{200, 0, 200});
    }
};

class Enemy2SetupSystem : public SetupSystem {
  public:
    void run() {
      const auto worldComponent = scene->world->get<WorldComponent>();
      int spriteSize = 48;
      ...
      scene->enemy = new Entity(scene->r.create(), scene);
      scene->enemy->addComponent<TransformComponent>(220, 220);
      auto& enemy2 = scene->enemy->addComponent<SpriteComponent>(
        "Sprites/Cat/SpriteSheet.png",
        0, 8,
        spriteSize,
        4,
      ...
      enemy2.lastUpdate = SDL_GetTicks();
      scene->enemy->addComponent<ColliderComponent>(19, 22, 10, 10, SDL_Color{200, 0, 200});
    }
};
